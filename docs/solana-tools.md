...
> 📌 为什么不做「按链上持仓快照空投」：那需要把该代币的**全部**持币账户拉下来（全量扫描），一个中等热度的币就是几十万个账户、几十 MB，浏览器和 RPC 都扛不住。快照请用你自己的数据源（或专门的索引服务）准备好地址清单，再导入这里。
...
> 📌 **为什么只看前 20 大**：完整持仓分布需要扫描该代币的全部账户（全量扫描）。实测一个中等热度的币就有 20 万个代币账户、56MB 数据、40 秒才跑完——那是索引器干的活，浏览器吃不下，而且这类「索引类查询」按量计费，非常吃节点额度。所以这里退回到「前 20 大」这个又便宜、又能回答"筹码是不是攥在少数人手里"的口径，**代价是没有「持币总人数」**（那个数字只有索引服务能给）。
>
> 池子识别靠「这个地址的账户所有者是不是系统程序」判断：普通钱包是系统账户，池子金库由各自的 DEX 程序持有。**不做老鼠仓 / 捆绑分析**——那需要历史成交与转账关系。
...
- **RPC 额度**：工具全部走浏览器直连的 Solana 节点，页面会在一个节点被限流时自动换下一个重试。代码**刻意不做全量扫描**、也避开了一切索引类方法（例如读代币余额用账户数据本地解码，而不是 `getTokenAccountBalance`），就是为了不把节点额度烧在没必要的地方。
