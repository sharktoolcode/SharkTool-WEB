...
- **工具**（开发中）：批量转账、快照持币、空投、归集、持币分析、权限检查。

### 📈 行情中心 `/market`

实时价格流、多时间框架 K 线、订单簿深度与技术指标——**开发中**。
...
| 服务 | 默认费用 |
|---|---|
| EVM 代币克隆 | 0.0001 BNB（各链可单独定价） |
| EVM 模块化发币 | 0.0001 BNB（各链可单独定价） |
| Solana 发币 | 0.01 SOL |
| Solana 建池 | 0.01 SOL |
...
| 商品 | 说明 | 价格 |
|---|---|---|
| 批量钱包生成与归集工具 | 批量派生地址、批量归集 | $90 / 0.15 BNB |
| 批量转账工具源码 | 含合约与前端，适合空投/分发 | $180 / 0.3 BNB |
| Solana 靓号钱包生成器 | GPU 加速，前后缀匹配 | $60 / 0.1 SOL |
| EVM 通用靓号地址生成器 | GPU 加速，任意前后缀 | $48 / 0.08 BNB |
| 波场 TRON 靓号地址生成器 | GPU 加速，私钥本地生成 | $58 / 0.1 BNB |
| 波场转账监控机器人 | 事件日志级监控，Telegram 实时推送 | $39 / 0.065 BNB |

- 支持 BSC BNB、BSC USDT、Tron USDT、Solana SOL、Solana USDT 支付。
- 每笔订单分配独立收款地址，到账后自动解锁下载。
