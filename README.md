...
- **工具**：批量转账（SPL / SOL 清单式群发，指令按体积自动打包）、快照持币（导出任意代币的持币地址与数量）、空投（按快照比例或白名单，带进度与失败重试）、归集（多个钱包一把扫回主钱包）、持币分析（持币人数、筹码集中度、池子与合约地址识别）、权限检查（铸币权/冻结权是否已撤销）。批量类工具支持**钱包逐笔签名**或**私钥本地签名**两种方式。
